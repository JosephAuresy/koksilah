...
if selected_option == "Watershed models":
    ...
    if DATA_FILENAME.exists() and points_file_path.exists():
        ...
        # Identify and group duplicates by Row and Column
        duplicates = filtered_data.groupby(['Row', 'Column']).filter(lambda x: len(x) > 1)
        
        if not duplicates.empty:
            st.subheader("Box Plots for August Rates at Overlapping Points in Same Grid Square")
            
            # Loop through each unique grid square with duplicates and create a combined plot
            for (row, column), group_data in duplicates.groupby(['Row', 'Column']):
                point_names = ', '.join(group_data['name'].unique())  # Get names of all points in this grid
                fig = px.box(
                    group_data,
                    x='Year',
                    y='Rate',
                    color='id',  # Differentiate points by ID
                    title=f"Box Plot of August Rates for Points in Grid Square ({row}, {column}) - {point_names}",
